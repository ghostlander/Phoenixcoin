...
Value stop(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "Phoenixcoin daemon has been stopped";
}
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (params.size() == 0)
    {
        ...
        result.push_back(Pair("target",   HexStr(BEGIN(hashTarget), END(hashTarget))));
        result.push_back(Pair("algorithm", "scrypt:1024,1,1"));  // use the standard scrypt algorithm
        return result;
    }
    else
    {
...
// Phoenixcoin: send alert (imported from PPC/XPM)
...
//
// JSON-RPC protocol. Phoenixcoin speaks version 1.0 for the maximum compatibility,
...
void ThreadRPCServer2(void* parg)
{
    ...
    ip::tcp::endpoint endpoint(bindAddress, GetArg("-rpcport", 9554)); // Phoenixcoin: the default RPC port is 9554
